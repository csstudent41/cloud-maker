...
\begin{tikzpicture}[
		...
		decision/.style={diamond, draw=black, fill=black!20},
	]
	\node[start] at (0, 0) (start) {};
	\node[fixlabel, below of=start] (browse) {Browse Files};
	\node[fixlabel, left of=browse, xshift=-2cm] (ofolder) {Open Folder};
	\node[fixlabel, right of=browse, xshift=2cm] (ofile) {Open File};
	...
	\node[decision, below of=delete, yshift=-0.5cm, fill=green!20] (conf1) {Confirm?};
